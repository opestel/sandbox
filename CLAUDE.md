...
- `style.css`'s base `body` rule sets `align-items: center; justify-content: center;` for the single-card layout used by `index.html`/`name-origin.html`. Any full-bleed page (like `map.html`, via `body.map-body`) must explicitly reset both back to `stretch`/`flex-start` — otherwise flex children without an explicit `width: 100%` silently shrink-wrap and center instead of spanning the viewport, which is easy to miss visually since `--bg` and `--card-bg` are close in tone.
- `countries.geojson` and `capitals.geojson` come from different upstream datasets and spell 8 country names differently (e.g. `United Republic of Tanzania` vs `Tanzania`, `Republic of Serbia` vs `Serbia`, the two Congos, Cabo Verde/Cape Verde, Guinea-Bissau/Guinea Bissau, Gambia/The Gambia, São Tomé and Principe/Sao Tome and Principe). The capital dot itself always renders fine (it's just lat/lng), but any lookup from a capitals-country-name into `countryLayers` (keyed by the boundaries name) needs `COUNTRY_NAME_ALIASES` in `map.js` — used today by the quiz's map-highlight step. `South Sudan` and `Palestine` have boundaries but no capital entry at all in the source data (not a naming issue, just missing).
